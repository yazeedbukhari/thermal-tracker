...
static HAL_StatusTypeDef amg8833_write8(I2C_HandleTypeDef *hi2c, uint8_t addr_7bit, uint8_t reg,
                                        uint8_t value)
{
    return HAL_I2C_Mem_Write(hi2c, AMG8833_HAL_ADDR(addr_7bit), reg, I2C_MEMADD_SIZE_8BIT, &value,
                             1U, AMG8833_I2C_TIMEOUT_MS);
}
...
HAL_StatusTypeDef AMG8833_Probe(I2C_HandleTypeDef *hi2c, uint8_t *detected_addr_7bit)
{
    ...
    for (uint32_t i = 0; i < 2U; i++) {
        HAL_StatusTypeDef st = HAL_I2C_IsDeviceReady(hi2c, AMG8833_HAL_ADDR(candidates[i]), 2U,
                                       ...
                                       uint8_t raw_frame[AMG8833_FRAME_BYTES])
{
    ...
    return HAL_I2C_Mem_Read(hi2c, AMG8833_HAL_ADDR(addr_7bit), AMG8833_REG_PIXEL_BASE,
                            I2C_MEMADD_SIZE_8BIT, raw_frame, AMG8833_FRAME_BYTES,
                                 ...
                                 float pixels_c[AMG8833_PIXEL_COUNT])
{
    ...
    for (uint32_t i = 0; i < AMG8833_PIXEL_COUNT; i++) {
        uint16_t raw = (uint16_t)raw_frame[2U * i] | ((uint16_t)raw_frame[(2U * i) + 1U] << 8);
