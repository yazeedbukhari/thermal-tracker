...
static void associate_ids(const ThermalObjectsResult *objs, uint32_t now_ms)
{
    ...
    for (uint8_t i = 0U; i < objs->count; i++) {
        ...
        for (uint8_t t = 0U; t < THERMAL_MAX_OBJECTS; t++) {
            if ((ctx.tracks[t].active == 0U) || (used[t] != 0U)) continue;
            if ((now_ms - ctx.tracks[t].last_seen_ms) > FSM_OBJ_TRACK_STALE_MS) continue;
            float d = absf(obj->centroid_x - ctx.tracks[t].cx) +
                      absf(obj->centroid_y - ctx.tracks[t].cy);
    ...
    for (uint8_t i = 0U; i < objs->count; i++) {
        const ThermalObject *obj = &objs->objects[i];
        if ((obj->valid == 0U) || (ctx.obj_ids[i] != OBJ_ID_NONE)) continue;
...
static void fill_detection(ThermalDetection *det,
                            ...
                            uint8_t idx)
{
    ...
    if ((idx == 0xFFU) || (idx >= objs->count) ||
        (objs->objects[idx].valid == 0U)) {
        det->target_found = 0U;
    ...
    det->min_x = obj->min_x;   det->max_x = obj->max_x;
    det->min_y = obj->min_y;   det->max_y = obj->max_y;
