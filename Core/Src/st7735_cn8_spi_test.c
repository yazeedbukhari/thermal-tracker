                                 ...
                                 uint8_t selected_idx)
{
    ...
    st7735_set_window(TFT_VIEW_X, TFT_VIEW_Y, (uint16_t)(TFT_VIEW_X + TFT_VIEW_W - 1U),
    ...
    for (uint8_t i = 0U; i < THERMAL_MAX_OBJECTS; i++) {
        ...
        if ((bx < 0) || (by < 0) || (bx >= (int16_t)ST7735_WIDTH) ||
            (by >= (int16_t)ST7735_HEIGHT)) {
            continue;
        }
        if ((bw <= 0) || (bh <= 0)) {
            continue;
        }
        if ((bx + bw) > (int16_t)ST7735_WIDTH)
            bw = (int16_t)ST7735_WIDTH - bx;
        if ((by + bh) > (int16_t)ST7735_HEIGHT)
            bh = (int16_t)ST7735_HEIGHT - by;

        if ((has_selected != 0U) && (i == selected_idx)) {
            st7735_rect((uint16_t)bx, (uint16_t)by, (uint16_t)bw, (uint16_t)bh, RGB565_LIME);
            st7735_draw_cross((int16_t)(TFT_VIEW_X + (obj->centroid_x * (float)scale_x)),
                              (int16_t)(TFT_VIEW_Y + (obj->centroid_y * (float)scale_y)),
                              RGB565_CYAN);
        } else {
